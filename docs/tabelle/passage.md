...
| **4** | Turns left 90° | Evento `turn`, disegnato come una **L da 3 celle (30ft)**: una cella nella direzione di prima, la cella d'angolo, una nella direzione nuova, e da lì il passaggio prosegue di là. Il braccio in ingresso viene allungato *prima* di ruotare se la gamba percorsa dall'ultima svolta è troppo corta (riga `[Layout] Il tiro non dava lunghezza propria...`); quello in uscita è un debito che paga il primo spostamento successivo, qualunque esso sia. Quanto vale "troppo corta" dipende dal verso delle due direzioni - vedi `_turn_lead` in `layout.py` e la sezione 8c di [`TODO.md`](../../TODO.md). Sopra il minimo la piega tiene comunque: una svolta dopo 50ft di corridoio resta una L. Poi ritira. |
| **5** | Turns right 90° | Come sopra, dall'altra parte. Vengo da sud e svolto a destra: cella 1 a nord di dove sono, cella 2 a nord di quella (aperta a sud e a est), cella 3 a est della 2 (aperta a ovest), e il passaggio prosegue a est. |
...
| **8** | Continues 1d4×10 ft, then a T-junction | `move`, poi **due** figli, sinistra e destra. A una T non c'è un "avanti": il passaggio finisce lì. Sulla mappa: il tratto dritto lungo quanto il tiro (30 ft = **3 celle**), e la cella su cui finisce ha **una cella a sinistra e una a destra**, il primo pezzo dei due bracci. Misurato su 40 semi: 66 T, 62 con tutti e due i bracci lì; nelle altre 4 un braccio va a sbattere in una stanza già disegnata, si ferma contro il muro e vi si apre come ingresso segreto, dicendolo nella propria voce. Un braccio che muore subito (limite della mappa, vicolo cieco) resta comunque disegnato come moncone di una cella, altrimenti la T non si vedrebbe. |
